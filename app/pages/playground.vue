...
<template>
  <div class="mx-auto max-w-screen-2xl p-4">
    ...
    <main class="rounded-md border border-border overflow-hidden">
      <ClientOnly>
      <div class="h-[70vh] w-full">
        ...
        >
          ...
          >
            ...
            <p class="text-muted-foreground text-xs pt-1">
              This popup has no marker — it's anchored to coordinates directly.
            </p>
          </MapPopup>
          <MapControls />
          <button
            v-if="!popupOpen"
            type="button"
            class="absolute top-2 left-2 z-10 rounded-md border border-border bg-background px-2.5 py-1 text-xs hover:bg-accent"
        ...
        >
          ...
          <MapMarker :longitude="routeCoords[0][0]" :latitude="routeCoords[0][1]">
            <MarkerContent>
              <div class="size-4 rounded-full border-2 border-white bg-emerald-500 shadow" />
          ...
          >
            <MarkerContent>
              <div class="size-4 rounded-full border-2 border-white bg-rose-500 shadow" />
        ...
        >
          <MapControls />
          <div
            class="absolute top-2 left-2 z-10 flex flex-col gap-1.5 rounded-md border border-border bg-background p-2 text-xs shadow"
          >
            <p class="font-medium">Controlled mode</p>
            <p class="text-muted-foreground">
              {{ viewport.center[0].toFixed(3) }},
              {{ viewport.center[1].toFixed(3) }} · z {{ viewport.zoom.toFixed(1) }}
            </p>
            <div class="flex gap-1.5">
              <button
                type="button"
                class="rounded-md border border-border bg-background px-2 py-0.5 hover:bg-accent"
                @click="flyToParis"
              >
                Paris
              </button>
      ...
      <template #fallback>
        <div class="grid h-[70vh] place-items-center text-muted-foreground">
          Loading…
        </div>
      </template>
      </ClientOnly>
